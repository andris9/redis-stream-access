    ...
    _write(chunk, encoding, callback) {
        ...
        let writeChunk = err => {
            ...
            snappy.compress(chunk, (err, compressed) => {
                ...
                this.client.rpush(this.key + ' stream', compressed, err => {
                    if (err) {
                        return callback(err);
                    }
                    callback();
            ...
            this.client.lindex(this.key + ' stream', this._index++, (err, chunk) => {
                ...
                snappy.uncompress(chunk, {
                    ...
                    if (this.push(data) !== false) {
                        ...
                        // need to wait until _read is called again
                        this.readingSource = false;
                    }
                });
            });
 ...
 */
function streamSetMeta(client, key, meta, callback) {
    client.exists(key + ' root', (err, exists) => {
        ...
        let data = Buffer.from(JSON.stringify(meta));
        snappy.compress(data, (err, compressed) => {
            if (err) {
                return callback(err);
            }

            client.set(key + ' meta', compressed, err => {
                if (err) {
                    return callback(err);
                }
    ...
    exec((err, replies) => {
        ...
        if (replies[2] && replies[2].length) {
            snappy.uncompress(replies[2], {
                ...
                try {
                    let data = JSON.parse(uncompressed);
